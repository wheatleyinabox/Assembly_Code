...
main:
	...
	add $s4, $zero, 1						# start count var $s4
	la $s3, 40						# load $s3 with arr length
...
o2:
	la $s6, 4						 # load $s6 with 4
	mult $s4, $s6						 # calculate current elements
	mflo $s5						 # (Mult count by 4)
	sub $s3, $s3, $s5					# subtract space from length to match current arr length
	...
	move $a0, $t1						# str store in print register
	syscall								# print value
	...
	li $v0, 1							# code for print_int
	move $a0, $s2						# str store in print register
	syscall								# print index
	...
	add $t5, $zero, $s3					# load $t5 with arr length
...
o3:
	...
	xor $s2, $s2, $s2
	and $s2, $s2, $0					# Reset count var $t2 to 0
...
isMax:
	...
	add $s3, $zero, $t5		# load new length into $s3
	j printMaxLoop
