...
#======================================================================================
		...
		lw $t8, 0($t6)				# $t8 <- value @ strArr[0]
		lw $t9, 0($t7)				# $t9 <- value @ strArr[1]
		sw $t8, 0($t7)				# strArr[1] <- $t8
		sw $t9, 0($t6)				# strArr[0] <- $t9

#======================================================================================

		li $s3, 4				# $s3 <- 4
		mult $s0, $s3 			# $t4 <- index int
		mflo $t4
		mult $s1, $s3				# $t5 <- index int
		mflo $t5
		...
		lw $t8, 0($t4)				# $t8 <- value @ ageArr[0]
		lw $t9, 0($t5)				# $t9 <- value @ ageArr[1]
		sw $t8, 0($t5)				# ageArr[1] <- $t8
		sw $t9, 0($t4)				# ageArr[0] <- $t9

		li $s6, 0					# clear registers $s6,
		li $t4, 0					# $t4,
		li $t5, 0					# $t5,
		li $t8, 0					# $t8,	
		li $t9, 0					# $t9 for reuse
...
#======================================================================================
		...
		lw $t8, 0($t4)				# $t8 <- value @ IDArr[0]
		lw $t9, 0($t5)				# $t9 <- value @ IDArr[1]
		sw $t8, 0($t5)				# IDArr[1] <- $t8
		sw $t9, 0($t4)				# IDArr[0] <- $t9
